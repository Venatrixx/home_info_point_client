...
Map interpretData(String htmlString) {
  ...
  final truncClassString = classString.substring(
    0,
    classString.indexOf(RegExp('[^0-9A-Z]')),
  );
  data['class'] = truncClassString;
  data['level'] = int.tryParse(
    truncClassString.substring(0, truncClassString.indexOf(RegExp('[^0-9]'))),
  );

  RegExp subjectTitlePattern = RegExp(
  ...
  for (int i = 0; i < htmlData.length; i++) {
    ...
    if (htmlData[i].localName == 'h3' &&
      ...
      if (htmlData[i].text == "Fehltage") {
        ...
        data['totalUnexcusedMissingDays'] = int.tryParse(
          rows[1].children[1].text,
        );

        data['totalMissingHours'] = int.tryParse(
          rows[2].children[1].text.substring(
            0,
            rows[2].children[1].text.indexOf(RegExp(r'[^0-9]')),
          ),
        );
        data['totalUnexcusedMissingHours'] = int.tryParse(
          rows[3].children[1].text.substring(
            0,
    ...
    if (htmlData[i].localName != 'h3' ||
        !subjectTitlePattern.hasMatch(
          htmlData[i].text.replaceAll(RegExp(r' '), ''),
        )) {
      continue;
    }
    ...
    subject['name'] = subjectTitle
        .substring(subjectTitle.indexOf('-') + 1, subjectTitle.lastIndexOf('('))
        .trim();
    subject['abbr'] = subjectTitle
        .substring(0, subjectTitle.indexOf('-'))
        .trim();
    ...
    for (int j = 0; j < rows.length; j++) {
      ...
      grade['isExam'] = ([
        ...
        'klassenarbeit',
        'ka',
      ].any(row.children[3].text.toLowerCase().contains));
      grade['semester'] = int.parse(
        row.children[4].text.substring(
          0,
          row.children[4].text.indexOf(RegExp('[^0-9]')),
        ),
      );
